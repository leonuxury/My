...
class Stock:
    ...
    def SendEmail(self):
        with SMTP_SSL(host='smtp.139.com') as smtp:
            smtp.login(user='li_jing_ok@139.com', password='LJlj1111')

            msg = MIMEText(f'请关注：{self.name} {self.ts_code}，当前处于盈利状态。', _charset="utf8")
            msg["Subject"] = f'请关注 {self.name}，当前处于盈利状态。'
            msg['From'] = 'li_jing_ok@139.com'  # 发送者
            msg['To'] = '1005411336@qq.com'  # 接收者

            smtp.sendmail(from_addr="li_jing_ok@139.com",
